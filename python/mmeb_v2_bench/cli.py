...
from .export_bundle import export_task_bundle, write_bundle_metadata
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="MMEB-V2 embedding and vector-quantization benchmark"
    )
    ...
    parser.add_argument("--export-bundle", type=Path, default=None, help="Export embedded vectors and labels for the C++ MMEB runner, then exit")
...
def _build_embedder(args: argparse.Namespace, cache: EmbeddingCache | None):
    ...
    return GeminiEmbedding2Embedder(
        GeminiEmbedderConfig(
            ...
            normalize=True,
            min_request_interval_seconds=float(os.environ.get("GEMINI_MIN_REQUEST_INTERVAL_SECONDS", "0")),
            max_request_interval_seconds=float(os.environ.get("GEMINI_MAX_REQUEST_INTERVAL_SECONDS", "60")),
            quota_backoff_seconds=float(os.environ.get("GEMINI_QUOTA_BACKOFF_SECONDS", "120")),
            max_quota_backoff_seconds=float(os.environ.get("GEMINI_MAX_QUOTA_BACKOFF_SECONDS", "600")),
            quota_interval_scale=float(os.environ.get("GEMINI_QUOTA_INTERVAL_SCALE", "1.5")),
        ),
        cache=cache,
    )
    ...
    try:
            ...
            if args.export_bundle is not None:
                ...
                write_bundle_metadata(
                    ...
                    payload={
                        ...
                        "embedder": str(args.embedder),
                        "gemini_model": str(args.gemini_model) if args.embedder == "gemini" else None,
                        "output_dim": int(args.output_dim),
                    },
                )
                print(json.dumps({"export_bundle": str(args.export_bundle), "tasks": [result.task_name]}, ensure_ascii=False))
                return
        ...
        if args.export_bundle is not None:
            ...
            for task_name in selected:
                ...
                print(json.dumps({"task": result.task_name, "task_dir": result.task_dir, "n_queries": result.n_queries, "n_candidates": result.n_candidates}, ensure_ascii=False))
            write_bundle_metadata(
                args.export_bundle,
                tasks=exported,
                train_xb=train_pool_xb,
            ...
            print(json.dumps({"export_bundle": str(args.export_bundle), "tasks": [task.task_name for task in exported]}, ensure_ascii=False))
            return

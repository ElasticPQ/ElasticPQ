...
def _task_media_dir(media_root: Path, spec: TaskSpec) -> Path:
    if not spec.media_subdir:
        return media_root

    direct = media_root / spec.media_subdir
    if direct.exists():
        return direct

    rel = Path(spec.media_subdir)
    parts = rel.parts
    if parts:
        nested = media_root.joinpath(*parts[:-1], parts[-2], parts[-1]) if len(parts) >= 2 else media_root / parts[0] / parts[0]
        if nested.exists():
            return nested

        repeated_tail = media_root.joinpath(*parts, parts[-1])
        if repeated_tail.exists():
            return repeated_tail

    return direct


def _resolve_media_path(media_root: Path, relative_path: str | Path) -> Path:
    rel = Path(str(relative_path))
    direct = media_root / rel
    if direct.exists():
        return direct

    # MMEB image archives unpack under image-tasks/MMEB/<task>/...
    mmeb_direct = media_root / "MMEB" / rel
    if mmeb_direct.exists():
        return mmeb_direct
...
def _materialize_hf_parquet_dataset(
    ...
    *,
    ...
    parquet_paths = sorted(path.resolve() for path in task_dir.rglob("*.parquet"))
    if not parquet_paths:
        raise RuntimeError(
            f"failed to materialize parquet files for task={task_name!r} split={split!r} "
            f"from {annotation_source!r}: {last_error!r}"
        )
    downloaded_files = [str(path) for path in parquet_paths]
    ...
    *,
    ...
    for row_idx, row in enumerate(_iter_rows(dataset)):
        ...
        if parser in {"image_cls", "image_qa", "image_i2t"}:
            ...
            image_path = _resolve_media_path(media_root, str(row["qry_img_path"]))
        ...
        if parser == "image_t2i":
            ...
            for target_path, caption in zip(target_paths, target_captions):
                ...
                candidate = _candidate_from_image(
                    name=name,
                    image_path=_resolve_media_path(media_root, target_path),
                    caption=caption,
                )
                corpus_map.setdefault(candidate.name, candidate)
        ...
        if parser == "image_i2i_vg":
            prompt = join_prompt_text(str(row.get("qry_inst", "")), str(row.get("qry_text", "")))
            query_image = _resolve_media_path(media_root, str(row["qry_img_path"]))
            target_paths = [str(item) for item in row["tgt_img_path"]]
            target_captions = [str(item) for item in row["tgt_text"]]
            candidate_names = []
            for target_path, caption in zip(target_paths, target_captions):
        ...
        raise NotImplementedError(
            f"dataset parser {parser!r} is not implemented"
        )

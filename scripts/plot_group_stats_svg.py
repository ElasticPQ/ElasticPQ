...
class GroupStat:
    ...
    def dproxy_per_dim(self) -> float:
        if self.is_tail:
            raise ValueError("tail segments do not have D_proxy/d values")
        if self.ndims <= 0:
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--metric",
        choices=("auto", "proxy", "effective_residual"),
        default="auto",
        help="Which group-stats metric to render. Default: prefer effective_residual if present, otherwise proxy.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        choices=("auto", "on", "off"),
        default="auto",
        help="Append AREPQ additive residual tail segments. Default: auto from meta.method.",
    )
    parser.add_argument("--arepq-total-bits", type=int, default=None, help="Total AREPQ budget when forcing a tail.")
    parser.add_argument("--arepq-tail-bits", type=int, default=None, help="Bits per AREPQ tail stage when forcing a tail.")
    parser.add_argument("--arepq-tail-stages", type=int, default=None, help="Number of AREPQ tail stages when forcing a tail.")
    parser.add_argument("--tail-color", default="#9ca3af", help="Fill color for additive residual tail segments.")
    parser.add_argument("--tail-label", default="tail", help="Bottom label for additive residual tail segments.")
    parser.add_argument(
        "--tail-width-mode",
        choices=("bits", "dims"),
        default="bits",
        help="Tail width mode. bits reserves tail_bits/total_bits bar width; dims keeps pure dimension-proportional widths.",
    )
...
def parse_group_stats(text: str) -> List[QuantizerStats]:
    ...
    current_metric = "proxy"
    current_total_dims: int | None = None
    current_total_bits: int | None = None
        ...
        if current_stats:
            ...
            blocks.append(
                QuantizerStats(
                    ...
                    stats=current_stats,
                    total_dims=current_total_dims,
                    total_bits=current_total_bits,
                    budget_bits=current_total_bits,
                )
            )
            current_stats = []
    ...
    for line in text.splitlines():
        ...
        if header is not None:
            ...
            current_metric = str(header.group("metric") or "proxy")
            current_total_dims = int(header.group("total_dims"))
            current_total_bits = int(header.group("total_bits"))
            continue
...
def filter_blocks_by_metric(
    blocks: Sequence[QuantizerStats],
    metric: str,
) -> tuple[List[QuantizerStats], str]:
    metric_s = str(metric)
    if metric_s == "auto":
        metric_s = "effective_residual" if any(block.metric == "effective_residual" for block in blocks) else "proxy"
    selected = [block for block in blocks if block.metric == metric_s]
    if not selected:
        available = ", ".join(sorted({block.metric for block in blocks}))
        raise ValueError(f"No [group-stats] blocks found for metric={metric_s}; available metrics: {available}")
    return selected, metric_s


def extract_arepq_method(text: str) -> dict[str, object] | None:
    for line in text.splitlines():
        line_s = line.strip()
        if not line_s.startswith("meta.method "):
            continue
        payload = line_s[len("meta.method "):].strip()
        try:
            meta = json.loads(payload)
        except json.JSONDecodeError:
            continue
        family = str(meta.get("family", "")).lower()
        if family.startswith("arepq"):
            return meta
    return None


def resolve_arepq_tail_spec(
    args: argparse.Namespace,
    text: str,
) -> tuple[int, int, int] | None:
    mode = str(args.arepq_tail)
    if mode == "off":
        return None

    meta = extract_arepq_method(text)
    total_bits = args.arepq_total_bits
    tail_bits = args.arepq_tail_bits
    tail_stages = args.arepq_tail_stages

    if meta is not None:
        total_bits = int(total_bits if total_bits is not None else meta.get("total_bits", 0))
        tail_bits = int(tail_bits if tail_bits is not None else meta.get("tail_bits", 0))
        tail_stages = int(tail_stages if tail_stages is not None else meta.get("tail_stages", 1))
    elif mode == "auto":
        return None

    if total_bits is None or tail_bits is None:
        raise ValueError("AREPQ tail is forced but --arepq-total-bits/--arepq-tail-bits are missing.")
    if tail_stages is None:
        tail_stages = 1
    if total_bits <= 0 or tail_bits <= 0 or tail_stages <= 0:
        raise ValueError("AREPQ total bits, tail bits, and tail stages must be positive.")
    if total_bits <= tail_bits * tail_stages:
        raise ValueError("AREPQ total bits must exceed tail_bits * tail_stages.")

    return int(total_bits), int(tail_bits), int(tail_stages)


def append_arepq_tail(
    blocks: Sequence[QuantizerStats],
    *,
    total_bits: int,
    tail_bits: int,
    ...
    expected_main_bits = int(total_bits) - int(tail_bits) * int(tail_stages)
    appended = 0
    for block in blocks:
        if any(item.is_tail for item in block.stats):
            continue
        main_bits = sum(item.bits for item in block.stats)
        header_bits = block.total_bits if block.total_bits is not None else main_bits
        if main_bits != expected_main_bits or header_bits != expected_main_bits:
            continue

        next_gid = max((item.group_id for item in block.stats), default=-1) + 1
        total_dims = block.total_dims if block.total_dims is not None else sum(item.ndims for item in block.stats)
        for stage in range(int(tail_stages)):
            label = tail_label if tail_stages == 1 else f"{tail_label}{stage + 1}"
            block.stats.append(
                GroupStat(
                    group_id=next_gid + stage,
                    ndims=int(total_dims),
                    bits=int(tail_bits),
                    dproxy=0.0,
                    is_tail=True,
                    label=f"{tail_bits}",
                    dim_label=label,
                    color_override=tail_color,
                )
            )
        block.budget_bits = int(total_bits)
        appended += 1
    if appended == 0:
        print(
            "warning: AREPQ tail requested but no block matched "
            f"expected_main_bits={expected_main_bits}",
            file=sys.stderr,
        )
...
def segment_widths(
    ...
    tail_items = [item for item in stats if item.is_tail]
    main_items = [item for item in stats if not item.is_tail]
    total_dims = sum(item.ndims for item in main_items if item.ndims > 0)
    if total_dims <= 0:
        raise ValueError("Total main ndims must be positive.")

    if tail_items and str(tail_width_mode) == "bits":
        total_budget = int(budget_bits) if budget_bits is not None else sum(item.bits for item in stats)
        if total_budget <= 0:
            raise ValueError("Total budget bits must be positive when drawing AREPQ tail widths.")
        tail_budget = sum(item.bits for item in tail_items)
        tail_usable = usable * float(tail_budget) / float(total_budget)
        main_usable = usable - tail_usable
        if main_usable <= 0.0 or tail_usable <= 0.0:
            raise ValueError("Invalid AREPQ tail/main width split.")
        raw = []
        for item in stats:
            if item.is_tail:
                raw.append(tail_usable * item.bits / tail_budget)
            else:
                raw.append(main_usable * item.ndims / total_dims)
    else:
        raw = [usable * item.ndims / sum(item.ndims for item in stats) for item in stats]
...
def build_svg(
    ...
    *,
    ...
    widths = segment_widths(
        ...
        gap=float(gap),
        tail_width_mode=tail_width_mode,
    ...
    for item, seg_w in zip(stats, widths):
        color = item.color_override if item.color_override is not None else dproxy_to_color(item.dproxy_per_dim, d_min=d_min, d_max=d_max)
        bit_label = item.label if item.label is not None else str(item.bits)
        dim_label = item.dim_label if item.dim_label is not None else f"{item.ndims}d"
        parts.append(
...
def main(argv: Sequence[str]) -> int:
    ...
    blocks, metric_used = filter_blocks_by_metric(blocks, str(args.metric))
    tail_spec = resolve_arepq_tail_spec(args, text)
    if tail_spec is not None:
        total_bits, tail_bits, tail_stages = tail_spec
        append_arepq_tail(
            blocks,
            total_bits=total_bits,
            tail_bits=tail_bits,
            tail_stages=tail_stages,
            tail_color=str(args.tail_color),
            tail_label=str(args.tail_label),
        )
    ...
    for block in blocks:
        ...
        svg = build_svg(
            ...
            tail_width_mode=str(args.tail_width_mode),
            budget_bits=block.budget_bits,
        )
    ...
    if range_mode_used == "quantile":
        print(
            ...
            f"(metric={metric_used} mode=quantile q_low={float(args.quantile_low):.3f} q_high={float(args.quantile_high):.3f})"
        )
    else:
        print(f"using D/d range: min={d_min:.10f} max={d_max:.10f} (metric={metric_used} mode={range_mode_used})")
    return 0

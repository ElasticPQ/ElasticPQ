...
MATRIX_DIR="${1:?usage: run_sift128_architecture_only_gate_shard.sh MATRIX_DIR SHARD_INDEX SHARD_COUNT}"
...
case_index=0
while IFS=$'\t' read -r case_id architecture init_seed structure_path config_path; do
  ...
  echo "start case=$case_id architecture=$architecture init=matched_physical/$init_seed log=$log"
  (
    echo "experiment=sift128_architecture_only_gate"
    echo "case_id=$case_id"
    echo "architecture=$architecture"
    echo "init_mode=matched_physical"

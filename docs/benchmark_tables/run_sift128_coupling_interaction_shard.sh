...
MATRIX_DIR="${1:?usage: run_sift128_coupling_interaction_shard.sh MATRIX_DIR SHARD_INDEX SHARD_COUNT}"
...
case_index=0
while IFS=$'\t' read -r case_id cell transform_seed structure_path config_path; do
  ...
  echo "start case=$case_id cell=$cell seed=$transform_seed log=$log"
  (
    echo "experiment=sift128_coupling_interaction"

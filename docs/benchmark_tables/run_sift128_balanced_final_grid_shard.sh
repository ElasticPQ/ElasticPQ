...
MATRIX_DIR="${1:?usage: run_sift128_balanced_final_grid_shard.sh MATRIX_DIR SHARD_INDEX SHARD_COUNT}"
...
case_index=0
while IFS=$'\t' read -r case_id m allocation transform_niter structure_path config_path; do
  ...
  if [[ -f "$log" ]] &&
     rg -q '^\s*Exit status: 0$' "$log" &&
     ! rg -q '^Command terminated by signal' "$log" &&
     rg -q "^transform_niter=${transform_niter}$" "$log" &&
     rg -q 'transform\.final_holdout .* exact_mse' "$log" &&
     rg -q '^\s*reconstruction error \(sample\):' "$log"; then
    echo "skip_complete case=$case_id log=$log"
    continue
  fi

  echo "start case=$case_id M=$m allocation=$allocation niter=$transform_niter log=$log"
  (
    echo "experiment=sift128_balanced_final_grid"

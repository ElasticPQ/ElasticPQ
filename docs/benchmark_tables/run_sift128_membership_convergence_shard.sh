...
MATRIX_DIR="${1:?usage: run_sift128_membership_convergence_shard.sh MATRIX_DIR SHARD_INDEX SHARD_COUNT}"
...
case_index=0
while IFS=$'\t' read -r case_id protocol membership membership_seed transform_niter structure_path config_path; do
  ...
  echo "start case=$case_id protocol=$protocol membership=$membership niter=$transform_niter log=$log"
  (
    echo "experiment=sift128_membership_convergence"

...
MATRIX_DIR="${1:?usage: run_sift128_membership_rotation_shard.sh MATRIX_DIR SHARD_INDEX SHARD_COUNT}"
...
case_index=0
while IFS=$'\t' read -r case_id protocol membership membership_seed init_mode init_seed structure_path config_path; do
  ...
  echo "start case=$case_id protocol=$protocol membership=$membership init=$init_mode/$init_seed log=$log"
  (
    echo "experiment=sift128_membership_rotation"
    echo "case_id=$case_id"
    echo "protocol=$protocol"
    echo "membership=$membership"
    echo "membership_seed=$membership_seed"
